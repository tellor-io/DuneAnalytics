...
SELECT token, count(*)
FROM(
  SELECT token FROM submission1
  UNION ALL
  SELECT token FROM submission2
  UNION ALL
  SELECT token FROM submission3
  UNION ALL
  SELECT token FROM submission4
  UNION ALL
  SELECT token FROM submission5
)
AS submissions
GROUP BY submissions.token
HAVING count(*)>1
